...
import argparse # this is what allows script to accept cli arguments from Ruby
import shutil
import subprocess
import sys
...
def main():
    ...
    result = subprocess.run(["python3", "-m", "demucs", "--two-stems", "vocals", "-o", str(output_dir), str(audio_path)],
    capture_output=True,
    text=True
    )
